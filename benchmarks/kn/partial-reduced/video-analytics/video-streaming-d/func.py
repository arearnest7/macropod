...
def Recognise(workflow_id, workflow_depth, frame):
    result = requests.post(os.environ['VIDEO_RECOG_PARTIAL'], json={"frame": base64.b64encode(frame).decode(), "workflow_id": workflow_id, "workflow_depth": workflow_depth + 1, "workflow_width": 0}).text
...
def main(context: Context):
    if 'request' in context.keys():
        ...
        videoFile.close()
        ret = Decode({"video": base64.b64encode(videoFragment).decode(), "workflow_id": workflow_id, "workflow_depth": workflow_depth, "workflow_width": 0})
