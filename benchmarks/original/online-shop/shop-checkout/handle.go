...
func quoteShipping(var address, var items) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-currency")
        ...
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {requestType: "quote", Address: address, Items: items})
...
func getUserCart(var userID) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-currency")
        ...
        shop-currency := string(contents)
        requestURL := shop-currency + ":80"
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {requestType: "get", UserID: userID})
	resp, err := http.DefaultClient.Do(req_url)
	if err != nil {
...
func emptyUserCart(var userID) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-cart")
        ...
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {requestType: "empty", UserId: userID})
	resp, err := http.DefaultClient.Do(req_url)
	if err != nil {
...
func prepOrderItems(var items) {
	...
	for i, item := range items {
        	req_url, err := http.NewRequest(http.MethodPost, requestURL, {&pb.GetProductRequest{Id: item.GetProductId()})
...
func convertCurrency(var from, var toCurrency) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-currency")
        ...
        shop-currency := string(contents)
        requestURL := shop-currency + ":80"
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {From: from, ToCode: toCurrency})
	result, err := http.DefaultClient.Do(req_url)
	if err != nil {
...
func chargeCard(var amount, var paymentInfo) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-payment")
        if err != nil {
                log.Fatal(err)
        }
        shop-payment := string(contents)
        requestURL := shop-payment + ":80"
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {Amount: amount, CreditCard: paymentInfo})
	paymentResp, err := http.DefaultClient.Do(req_url)
	if err != nil {
...
func sendOrderConfirmation(var email, var order) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-email")
        if err != nil {
                log.Fatal(err)
        }
        shop-email := string(contents)
        requestURL := shop-email + ":80"
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {Email: email, Order: order})
	resp, err := http.DefaultClient.Do(req_url)
	if err != nil {
...
func shipOrder(var address, var order) {
	contents, err := ioutil.ReadFile("/etc/secret-volume/shop-shipping")
        ...
        shop-shipping := string(contents)
	requestURL := shop-shipping + ":80"
        req_url, err := http.NewRequest(http.MethodPost, requestURL, {requestType: "order", Address: address, Items: items})
        resp, err := http.DefaultClient.Do(req_url)
	if err != nil {
                return "", fmt.Errorf("shipment failed: %+v", err)
        }
