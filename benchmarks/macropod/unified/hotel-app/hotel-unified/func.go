...
func CheckAvailability(req map[string]interface{}) string {
    ...
    for _, hotelId := range req["HotelIds"].([]string) {
        ...
        inDate, _ := time.Parse(
            time.RFC3339,
            req["InDate"].(string)+"T12:00:00+00:00")

        outDate, _ := time.Parse(
            time.RFC3339,
            req["OutDate"].(string)+"T12:00:00+00:00")
        ...
        for inDate.Before(outDate) {
            ...
            if count+int(req["RoomNumber"].(int)) > hotel_cap {
                break
            }
            indate = outdate
...
func MakeReservation(req map[string]interface{}) string {
    ...
    for inDate.Before(outDate) {
        ...
        if err == nil {
            ...
            memc_date_num_map[memc_key] = count + req["RoomNumber"].(int)
        ...
        if count+req["RoomNumber"].(int) > hotel_cap {
...
func GetProfiles(req map[string]interface{}) string {
    ...
    for _, i := range req["HotelIds"].([]string) {
...
func SearchNearby(req map[string]interface{}) string {
    // find nearby hotels
...
func FunctionHandler(context Context) (string, int) {
    ...
    if body_u["Request"].(string) == "search" {
        ret_val = SearchNearby(body_u)
    } else if body_u["Request"].(string) == "recommend" {
        ret_val = GetRecommendations(body_u)
    } else if body_u["Request"].(string) == "reserve" {
        if body_u["RequestType"].(string) == "check" {
            ret_val = CheckAvailability(body_u)
        } else if body_u["RequestType"].(string) == "make" {
            ret_val = MakeReservation(body_u)
        }
    } else if body_u["Request"].(string) == "user" {
        ret_val = strconv.FormatBool(CheckUser(body_u))
    } else if body_u["Request"].(string) == "profile" {
        ret_val = GetProfiles(body_u)
    }
