...
func Serve_WF_Invoke(res http.ResponseWriter, req *http.Request) {
    ...
    payload_interface := make(map[string]interface{})
    json.Unmarshal(body, &payload_interface)
    payload, _ := structpb.NewStruct(payload_interface)
    ...
    start_time := time.Now()
    dw := int32(0)
    response, err := stub.Invoke(context.Background(), &pb.FunctionRequest{JSON: payload, WorkflowID: &workflow_id, Depth: &dw, Width: &dw})
    end_time := time.Now()
