...
def RPC(context, dest, payloads):
    ...
    with futures.ThreadPoolExecutor(max_workers=len(payloads)) as executor:
        for i in range(len(payloads)):
            payload = payloads[i]
            tl.append(executor.submit(requests.post, url=dest, json={"Data": payload.decode("utf-8"), "WorkflowId": context["WorkflowId"], "Depth": (context["Depth"] + 1), "Width": i, "RequestType": request_type}))
    results = []
