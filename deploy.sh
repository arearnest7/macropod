...
for i in ${worker_nodes[@]}; do ssh $user@$i -tt "sudo curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="agent --server https://$host:6443 --token $token --flannel-iface $iface --node-external-ip $worker" sudo sh -"; done;
host_name=$(hostname)
