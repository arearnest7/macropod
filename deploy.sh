...
for i in ${worker_nodes[@]}; do echo $passwd | sshpass -p $passwd ssh $user@$i -tt "sudo apt-get install ca-certificates curl gnupg && curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC=\"agent --server https://$host:6443 --token $token --flannel-iface=$iface --node-external-ip $i\" sh -"; done;
host_name=$(hostname)
sudo kubectl taint nodes $host_name master-node=master-node:NoSchedule
sed -i "s/value: \"username\"/value: \"$user\"/g" deployment/macropod.yaml
sed -i "s/value: \"password\"/value: \"$passwd\"/g" deployment/macropod.yaml
...
sed -i "s/value: \"$user\"/value: \"username\"/g" deployment/macropod.yaml
sed -i "s/value: \"$passwd\"/value: \"password\"/g" deployment/macropod.yaml
sed -i "s/\"value: \"$worker_nodes\"/value: \"192.168.56.21 192.168.56.22 192.168.56.23 192.168.56.24\"/g" deployment/macropod.yaml
