...
sleep 180s
hey -n $N -c $C -t 180 -o csv -D $PAYLOAD -m POST -T application/json http://10.43.190.1/$BENCHMARK >> ../../collection/benchmark-collection/macropod-$BENCHMARK.csv
curl http://10.43.190.1/logs/$BENCHMARK >> ../../collection/benchmark-collection/macropod-$BENCHMARK-log-bundle.csv
