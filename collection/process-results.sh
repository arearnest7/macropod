...
for concurrency in ${c[@]}; do
	...
	go run processing-scripts/process-latency.go 1000 ${LATENCY_RESULTS[$concurrency]} $DIR $LATENCY_FILES
done;
...
go run processing-scripts/process-metrics.go $METRIC_RESULTS $METRIC_FILES
...
go run processing-scripts/process-stats.go $STATS_RESULTS $METRIC_RESULTS $DIR $LATENCY_STR
