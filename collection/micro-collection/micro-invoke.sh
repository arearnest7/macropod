...
sleep 180s
hey -n $N -c $C -t 1000 -d "{}" -o csv -m POST -T application/json http://micro-rpc-a.default.$HOST.sslip.io >> micro-rpc-kn-original-$DEST.csv
...
sleep 180s
hey -n $N -c $C -t 1000 -d "{}" -o csv -m POST -T application/json http://micro-rpc-a-b.default.$HOST.sslip.io >> micro-rpc-kn-full-$DEST.csv
